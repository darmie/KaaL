...
export def "codegen memory-config" [platform_cfg: record] {
    ...
    # Calculate virtual memory layout addresses from offsets/sizes
    # Convert hex strings from TOML to integers, then do arithmetic
    let user_virt_start_int = ($platform_cfg.user_virt_start | into int)
    let loader_offset_int = ($platform_cfg.loader_virt_offset | into int)
    let loader_size_int = ($platform_cfg.loader_virt_size | into int)
    let ipc_size_int = ($platform_cfg.ipc_virt_size | into int)

    let loader_virt_start = ($user_virt_start_int + $loader_offset_int)
    let loader_virt_end = ($user_virt_start_int + $loader_offset_int + $loader_size_int)
    let ipc_virt_start = $loader_virt_end
    let ipc_virt_end = ($loader_virt_end + $ipc_size_int)
    let user_dynamic_virt_start = $ipc_virt_end
...
/// Component loader region start \(calculated: user_virt_start + loader_virt_offset\)
pub const LOADER_VIRT_START: u64 = ($loader_virt_start);

/// Component loader region end \(calculated: LOADER_VIRT_START + loader_virt_size\)
pub const LOADER_VIRT_END: u64 = ($loader_virt_end);

/// IPC shared memory region start \(calculated: LOADER_VIRT_END\)
pub const IPC_VIRT_START: usize = ($ipc_virt_start) as usize;

/// IPC shared memory region end \(calculated: IPC_VIRT_START + ipc_virt_size\)
pub const IPC_VIRT_END: usize = ($ipc_virt_end) as usize;

/// Dynamic memory allocation start \(calculated: IPC_VIRT_END\)
pub const USER_DYNAMIC_VIRT_START: u64 = ($user_dynamic_virt_start);
...
export def "codegen roottask-linker" [platform_cfg: record, root_task_stack_size: string] {
    ...
    # Use user virtual address space, not physical memory offset
    # The kernel will map the ELF segments to user virtual addresses
    let roottask_virt_base = ($platform_cfg.user_virt_start | into int)
...
SECTIONS
{
     ...
     * Root task loads in user space, not physical memory
     */
    . = (printf '0x%x' $roottask_virt_base);
